...
export const StepComponent: React.FC<StepComponentProps> = ({ number, step, currentStep }) => {
  ...
  return (
    <div className="flex flex-row items-center gap-5">
      ...
      {number < currentStep 
        ? <div className="w-[40px] h-[40px] bg-blue-500 rounded-full flex justify-center items-center">
            <Image src="/checkmark.png" width={15} height={15} alt="checkmark.png" />
          </div>
        : <div className={`border-2 ${number > currentStep ? "border-gray-500 text-gray-500" : "border-blue-500 text-blue-500"} rounded-full w-[40px] h-[40px] flex justify-center items-center`}>
            {number}
          </div>
      }
